...
export const getAllPostsController = async (req: Request, res: Response) => {
  ...
  if (!safeData.success) return res.status(400).json(safeData.error);
...
export const updatePostController = async (req: Request, resp: Response) => {
  ...
  if (!safeData.success) return resp.status(400).json(safeData.error);
  ...
  if (!post) return resp.status(400).json({ message: "Invalid post id" });
...
export const getPostBySlugController = async (req: Request, resp: Response) => {
  ...
  if (!slug) return resp.status(400).json({ message: "Invalid slug" });
